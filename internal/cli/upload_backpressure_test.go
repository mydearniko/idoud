...
func TestUploadPUTRetainsBackpressureRetryAfterWithoutBlockingRoute(t *testing.T) {
	...
	failIf(t, status != http.StatusServiceUnavailable, "status=%d, want 503", status)
	var reqErr *requestError
	failIf(t, !errors.As(uploadErr, &reqErr) || reqErr == nil, "error=%v, want requestError", uploadErr)
	...
	fatalIf(t, u.routes == nil || !u.routes.available(target.String(), time.Now()), "backpressure response opened the route circuit")
	fatalIf(t, u.cooldowns == nil || u.cooldowns.remaining(uploadRouteTarget{rawURL: target.String()}, time.Now()) < 200*time.Millisecond, "backpressure response did not establish the shared Retry-After cooldown")
}
